...
@section('section')
    ...
    <div class="w-full lg:w-9/12">
        <form role="form" method="post" action="{{"/"}}ispitniRok/{{$ispitniRok->id}}">
            {{csrf_field()}}
            {{method_field('PATCH')}}

            <x-card variant="success">
                <x-slot:header>
                    <h3 class="text-lg font-semibold">Измени испитни рок</h3>
                </x-slot:header>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <x-form-input label="Назив:" name="naziv" type="text" value="{{$ispitniRok->naziv}}" />
