            ...
            <form method="POST" action="{{ url('/password/reset') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="email" class="mt-1 block w-full rounded-lg border-secondary-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm @if($errors->has('email')) border-danger-300 @endif" name="email" value="{{ $email or old('email') }}">
                        @if ($errors->has('email'))
