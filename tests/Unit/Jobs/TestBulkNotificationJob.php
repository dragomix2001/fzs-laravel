...
class TestBulkNotificationJob extends TestCase
{
    private string $jobClass = 'App\\Jobs\\BulkNotificationJob';
    ...
    public function test_job_handles_data_correctly_through_serialization_cycle(): void
    {
        $job = $this->makeJobInstance();
    ...
    public function test_job_specific_methods_or_properties_are_present_and_valid(): void
    {
        $job = $this->makeJobInstance();
        $reflection = new ReflectionClass($job);

        foreach (['tries', 'timeout', 'queue'] as $property) {
    ...
    private function fakeValueForParameter(ReflectionParameter $parameter): mixed
    {
        ...
        if (str_contains($name, 'user') || str_contains($name, 'recipient')) {
            return [1, 2, 3];
        }
        ...
        if (str_contains($name, 'message') || str_contains($name, 'body')) {
